(function() {
	...
	pMaterial = new THREE.ParticleBasicMaterial({
	    //color: fgColour,
	    vertexColors:true,
	    size: useEvery * 3.5 // magic number?
	});
	...
	console.log("no faces: " + particles.faces.length);
	...
	for (y=0; y<h-1; y+=10)
	{
		...
		yw1=(y+1)*w;
		x1=x+1;
		particles.faces.push(new THREE.Face3(yw+x,yw+x1,yw1+x));
		particles.faces.push(new THREE.Face3(yw1+x1,yw1+x,yw+x1));
	    }
	}
	console.log("added faces: " + particles.faces.length);

	// particles.computeBoundingSphere(); 

	particleSystem = new THREE.ParticleSystem(particles, pMaterial);
	//particleSystem = new THREE.Mesh(particles);;
	//particleSystem = new THREE.Mesh(particles);
...
//	    console.log("in animate");
	...
	// callback for data from websocket
	var start=0;
...
//	    console.log("in data callback: " + e.data.byteLength);
	    ...
	    // if (keyFrame)
	    // 	console.log("it is a keyframe");
	    // else
	    // 	console.log("it is not a keyframe");
	    ...
	    var jump=1;
	    start = (start+1)%jump;
	    for (pIdx = start, byteIdx=5+start; pIdx < pvs.length; pIdx+=jump)
	    {
	    	    pv = pvs[pIdx];
	    	    aByte = bytes[byteIdx];
