...
import sys, signal, numpy, freenect, pylzma, time
...
class Kinect:
  ...
  def bodyCallback(self, *args):
    if self.videoReady and self.depthReady:
...
#      crunchedData = pylzma.compress(data, dictionary = 18, algorithm = 0)  # default: 23 -> 2 ** 23 -> 8MB
      reactor.callFromThread(self.wsFactory.broadcast, data.tostring(), True)
      
      end= time.clock()
      elapsed= (end-start)

      print 'sending data took %s seconds' % elapsed
